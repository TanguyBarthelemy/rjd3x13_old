...
#' @param lsigma numeric: the lower sigma boundary for the detection of extreme values.
#' @param usigma numeric: the upper sigma boundary for the detection of extreme values.
#'
#' @param henderson.filter numeric: the length of the Henderson filter (odd number). If `henderson.filter = 0` an automatic selection of the Henderson filter's length
...
#' @param bcasts,fcasts numeric: the number of backasts (`bcasts`) or forecasts (`fcasts`) generated by the RegARIMA model in periods (positive values) or years (negative values).
...
#' @param exclude.forecast Boolean to exclude forecasts and backcasts. If `TRUE`, the RegARIMA model forecasts and backcasts are not used during the detection of extreme values in the seasonal adjustment routines.
#' @param bias TODO.
#'
